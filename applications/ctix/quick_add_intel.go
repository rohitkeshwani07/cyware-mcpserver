...
func CreateQuickAddIntel(payload any, headers map[string]string) (*common.APIResponse, error) {
	// implementation of quick add intel
	quick_add_resp := &QuickAddIntelResponse{}
	resp, err := CTIX_CLIENT.MakeRequest("POST", Quick_add_intel_create, nil, quick_add_resp, payload, headers)
...
func CreateQuickAddIntelTool(s *server.MCPServer) {
	...
	s.AddTool(quickAddIntelCreateTool, func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		resp, err := CreateQuickAddIntel(request.Params.Arguments, headers)
